 ...
 */
export const executeRayBuy = async (
  wallets: WalletRayBuy[],
  tokenConfig: TokenConfig,
  customAmounts?: number[]
): Promise<{ success: boolean; result?: any; error?: string }> => {
  try {
    console.log(`Preparing to ray buy ${tokenConfig.tokenAddress} using ${wallets.length} wallets`);
    ...
    console.error('Ray buy error:', error);
    return {
      success: false,
      error: error.message
    };
