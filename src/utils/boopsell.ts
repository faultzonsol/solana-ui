 ...
 */
const splitBundlesIntoMaxSize = (bundles: BoopSellBundle[]): BoopSellBundle[] => {
  const result: BoopSellBundle[] = [];
...
/**
 * Get partially prepared boop sell transactions from backend
...
/**
 * Execute boop sell operation
 */
export const executeBoopSell = async (
  wallets: WalletBoopSell[],
  tokenConfig: TokenSellConfig
): Promise<{ success: boolean; result?: any; error?: string }> => {
  try {
    console.log(`Preparing to boop sell ${tokenConfig.tokenAddress} using ${wallets.length} wallets, sell percentage: ${tokenConfig.sellPercent}%`);
    ...
    console.error('Boop sell error:', error);
    return {
      success: false,
      error: error.message
    };
...
/**
 * Validate boop sell inputs
 */
export const validateBoopSellInputs = (
  wallets: WalletBoopSell[],
  tokenConfig: TokenSellConfig,
