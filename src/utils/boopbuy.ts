...
/**
 * Get partially prepared boop buy transactions from backend
 ...
 */
const splitLargeBundles = (bundles: BoopBuyBundle[]): BoopBuyBundle[] => {
  const result: BoopBuyBundle[] = [];
...
/**
 * Execute boop buy operation
 */
export const executeBoopBuy = async (
  wallets: WalletBoopBuy[],
  tokenConfig: TokenConfig,
  customAmounts?: number[]
): Promise<{ success: boolean; result?: any; error?: string }> => {
  try {
    console.log(`Preparing to boop buy ${tokenConfig.tokenAddress} using ${wallets.length} wallets`);
    ...
    console.error('Boop buy error:', error);
    return {
      success: false,
      error: error.message
    };
...
/**
 * Validate boop buy inputs
 */
export const validateBoopBuyInputs = (
  wallets: WalletBoopBuy[],
  tokenConfig: TokenConfig,
