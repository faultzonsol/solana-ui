...
/**
 * Execute ray sell operation.
 */
export const executeRaySell = async (
  wallets: WalletRaySell[],
  tokenConfig: TokenConfig
): Promise<{ success: boolean; result?: any; error?: string }> => {
  try {
    console.log(`Preparing to ray sell ${tokenConfig.sellPercent}% of ${tokenConfig.tokenAddress} using ${wallets.length} wallets`);
    ...
    console.error('Ray sell error:', error);
    return {
      success: false,
      error: error.message
    };
...
/**
 * Validate ray sell inputs.
 */
export const validateRaySellInputs = (
  wallets: WalletRaySell[],
  tokenConfig: TokenConfig,
